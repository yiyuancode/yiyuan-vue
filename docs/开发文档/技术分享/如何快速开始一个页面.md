...
该例子传递了scopedSlots属性，值为3，表示查询的字段有3个，以及loading属性，是否显示loading效果，以及再传递一个search事件
...
上面的代码，当你每次需要新增一个查询项的时候，需要在外面套一层a-form-model-item，要加一个slot，值为scopedSlots-index，这里的index代表索引，有几项就需要写几项，里面的查询项可以使用ant-design-vue的表单组件 
...
- 找到y-search的组件，看查询的表单需要几项，然后进行配置，以及设置自己需要的查询表单的组件，通过插槽scopedSlots-index的形式（这样查询的表单就配置好了）
- 找到getData方法，把查询的api替换成你项目中后端的api
- 在onDelete方法里面，把删除的api替换成你项目中后端的api
- 在onBatchDelete方法里面，把批量删除的api替换成你项目中后端的api
- 在onSubmitHandle方法里面，把添加和编辑的api替换成你项目中后端的api
