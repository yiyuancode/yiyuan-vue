<template>
  <ManagePage ref="managePage" :columns="columns" :data="data" :pagination="pagination"
              :renderObj="renderObj"
              @onSave="saveHandle"
    ...
    <template slot="otherOperationsGOContainer">
      <AddForm ref="addForm" :props="addFormProps" @propsChange="addFormPropsChange"
               @propsSubmit="generateCodeSubmit"></AddForm>
      <a-button type="primary" @click="refreshClick">
        更新数据库表
      </a-button>
      <a-button type="primary" @click="generateCodeClick">
        生成代码
      </a-button>
    </template>
  </ManagePage>
          ...
          columns: [
            {
              ...
              rules: [{required: true, message: '请输入根包名', trigger: 'blur'},],
            ...
            },
            {
              ...
              rules: [{required: true, message: '请输入一级目录名称', trigger: 'blur'},],
