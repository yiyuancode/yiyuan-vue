...
<script>
  ...
  import {addMenu, deleteMenu, editMenu, getMenuDetail} from '@/api/sys/menu';
  import {getUserInfo} from '@/api/login';
  import menuForm from './menuForm';
  import Modal from '@/components/modal/Modal';
  import _ from 'lodash';
  import {getRoutes, loadRoutes} from '@/utils/routerUtil';
  ...
  const menuRules = {
    ...
    frameSrc: [{required: true, message: '请输入外部链接地址', trigger: 'blur'}]
  };
    ...
    methods: {
      ...
      async saveMenuHandle(opType) {
        ...
        try {
          ...
          const routes = await this.getRoutes(userInfo.menuTreeList);
          // 获取路由信息
          loadRoutes(routes);
      ...
      handleSubmitMenuInfo(menuInfo) {
        ...
        submitMenuInfo.parentId = submitMenuInfo.parentId
          ? submitMenuInfo.parentId
          : '0'; //如果没有就是0
