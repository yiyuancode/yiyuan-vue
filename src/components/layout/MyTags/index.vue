<template>
  <div style="width: 100%">
    ...
    >
      <a-tab-pane
        v-for="item in $store.getters.myTags"
        :key="item.name"
          ...
          <a-dropdown :trigger="['contextmenu']">
            <div class="tab">
              <a-icon
                ...
                :class="[
                  'icon-sync',
                  { hide: item.name != $store.state.myTagsActiveKey }
                ]"
                @click="refresh(item)"
              />
              <span v-if="item.name == $route.name">{{ item.meta.title }}</span>
              <span v-if="item.name != $route.name" @click="changeClik(item)">{{
                item.meta.title
              }}</span>
...
export default {
  ...
  methods: {
    ...
    onEdit(targetKey, action) {
      ...
      if (action == 'remove') {
        this.cloeClick(dynamicRoutesMap[targetKey]);
      }
    },
    myTagsChange(targetKey) {
      // this.myTagsActiveKey = targetKey;
      // console.log("onEdit.myTagsChange.myTagsActiveKey", this.myTagsActiveKey)
      this.changeClik(dynamicRoutesMap[targetKey]);
    }
  }
};
</script>
