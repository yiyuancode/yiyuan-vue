  ...
  <div class="MyTags">
    <template v-for="(item, index) in $store.getters.myTags">
      ...
      >
        ...
        <div class="tags-item" @click="cloeOtherClick(item, index)">
          关闭其他
        </div>
        ...
        <el-tag
          ...
          closable
          effect="plain"
          :class="{ myTags: item.choose }"
          :color="item.choose ? '#42B983' : '#FFFFFF'"
          type="info"
          @close="cloeClick(item)"
        >
          <span
            :style="{
              cursor: 'pointer',
              color: item.choose ? '#FFFFFF' : '#000000'
            }"
            @click="changeClik(item)"
            @contextmenu.prevent="rightClck(item)"
