    ...
    <a-row>
      <a-col :xxl="{ span: 20 }" :xl="{ span: 18 }" :lg="{ span: 24 }">
        <a-row class="search-form-row">
          <a-col
            v-for="(searchItem, index) in uSearchFormList"
            :key="index"
            :lg="{ span: 8 }"
            :md="{ span: 12 }"
          >
  ...
  methods: {
    ...
    getUSearchFormList() {
      this.uSearchFormList = this.searchFormList.map((searchItem) => {
        const { formType, title, options, key } = searchItem;
    ...
    searchForm() {
      ...
      this.uSearchFormList.forEach((searchFormItem) => {
        const { formType, key, value } = searchFormItem;
