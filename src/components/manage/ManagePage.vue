  ...
  <div class="manage-container">
    ...
    <div class="content-container">
      <div class="operate-btn-container">
        ...
        <a-popconfirm title="是否要批量删除这些信息?" ok-text="确认" cancel-text="取消"
      ...
      <div class="list-container">
        <a-table style="margin-bottom: 10px;" :columns="columnsObj.uTheadData" :data-source="data"
          ...
          <template v-for="(item, index) in columnsObj.tdColumnData"
                    :slot="item.scopedSlots?.customRender"
                    slot-scope="text,record">
            <!-- {{ record }} -->
            <slot :name="'table-' + item.scopedSlots?.customRender"
                  v-bind="{ text, record }"></slot>
          </template>

          <span slot="action" slot-scope="text, record">
                        <div v-if="uRenderObj.operateMode === 1" class="operate-btn-container">
                            <a-button v-if="uRenderObj.editBtn.isOpen" type="primary"
                          ...
                          <!-- 删除 -->
                            <a-popconfirm title="是否要删除这条信息?" ok-text="确认" cancel-text="取消"
                                ...
                                <a-button v-if="uRenderObj.deleteBtn.isOpen"
    ...
    <Modal :modalWidth="submitModalObj.modalWidth" :modalTitle="submitModalTitle"
           :modalVisible="modalVisble"
           :submitLoading="submitLoading" @onSubmit="submitHandle"
  ...
  export default {
    ...
    computed: {
      ...
      columnsObj() {
         ...
         */
        const addFormList = [], editFormList = [], objColumnsArr = [], uTheadData = [],
          tdColumnData = [], searchFormList = [];
