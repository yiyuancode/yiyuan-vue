      ...
      <a-form :form="form" :label-col="{ span: 5 }" :wrapper-col="{ span: 16 }">
        ...
        <template v-for="(item) in props.columns">
          ...
          <a-form-item v-if="!item.upload&&!item.noAdd" :label="item.title">
            <a-radio-group :disabled="item.disabled?item.disabled:false" :placeholder="item.title"
                           v-if="item.radio"
            ...
            <a-select :disabled="item.disabled?item.disabled:false"
                      v-else-if="item.select&&!item.select.search" allowClear show-search
                      :placeholder="item.title" option-filter-prop="children"
                      v-decorator="[item.dataIndex, { rules: [{ required: true, message: item.title }] }]">
              <a-select-option :value="im.value" v-for="( im) in item.select.option">
                {{im.name}}
              </a-select-option>
            ...
            <a-select :disabled="item.disabled?item.disabled:false"
                      v-else-if="item.select&&item.select.search " allowClear
                      :placeholder="item.title" show-search :value="value"
                      :default-active-first-option="false"
                      :filter-option="false"
                      v-decorator="[item.dataIndex, { rules: [{ required: true, message: item.title }] }]"
                      @search="(val)=>handleSearchSelect(val,item.dataIndex,item)"
                      @change="(val,option)=>handleChangeSelect(val,item.dataIndex,item,option)">
            ...
            <a-date-picker v-else-if="item.time" v-decorator="[item.dataIndex, {
        rules: [{  required: true, message: item.title }],
      }]" show-time format="YYYY-MM-DD HH:mm:ss"/>

            <a-input :disabled="item.disabled?item.disabled:false" :placeholder="item.title" v-else
                     v-decorator="[item.dataIndex, { rules: [{ required: true, message: item.title }] }]"
                     allowClear/>
      ...
      this.$nextTick(() => {
        ...
        propsTmep.columns.forEach(async (im, ix) => {
          ...
          if (im.select && im.select.methods && !im.select.remoteUrl) {
            im.select.option = await vm[im.select.methods.name](im.select.methods.parm);
            vm.$emit("propsChange", propsTmep)
          }
        ...
        if (propsTmep.type == 2) {
          ...
          if (Object.keys(propsTmep.record).length != 0) {
            setTimeout(() => {
              ...
              console.log("propsTmep.record.vm.form.getFieldsValue()", vm
                .form
                .getFieldsValue())
            }, 100)
          }
